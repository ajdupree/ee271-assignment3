...
module dff_unq13 (
   ...
   //   DW03_pipe_reg #(5,4) dff ( .A(in) , .clk(clk) , .B(out) ) ;
   DW_pl_reg #(.stages(6),.in_reg(0),.out_reg(0),.width(4),.rst_mode(0)) dff ( .data_in(in) , .clk(clk) , .data_out(out), .rst_n(!reset), .enable({5{en}}) );
