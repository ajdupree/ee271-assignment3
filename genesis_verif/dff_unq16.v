...
module dff_unq16 (
   ...
   //   DW03_pipe_reg #(4,1) dff ( .A(in) , .clk(clk) , .B(out) ) ;
   DW_pl_reg #(.stages(5),.in_reg(0),.out_reg(0),.width(1),.rst_mode(0)) dff ( .data_in(in) , .clk(clk) , .data_out(out), .rst_n(!reset), .enable({4{en}}) );
